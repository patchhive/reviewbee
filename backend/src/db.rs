...
fn open_connection() -> Result<Connection> {
    let conn = Connection::open(db_path()).context("Could not open ReviewBee database")?;
    conn.execute_batch("PRAGMA journal_mode=WAL; PRAGMA foreign_keys=ON;")
        .context("Could not initialize ReviewBee database pragmas")?;
    Ok(conn)
}
...
fn connect() -> Result<MutexGuard<'static, Connection>> {
    ...
    mutex
        .lock()
        .map_err(|_| anyhow!("ReviewBee database mutex poisoned"))
}
